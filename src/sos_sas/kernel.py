    ...
    def get_vars(self, names):
        ...
        for name in names:
            ...
            self.sos_kernel.run_cell(
                ...
                True,
                False,
                on_error='Failed to put variable {} to SAS'.format(name))
    ...
    def put_vars(self, items, to_kernel=None):
        ...
        try:
            for idx, item in enumerate(items):
                tabname = "'" + item.strip() + "'n "

                code = "data sasdata2dataframe / view=sasdata2dataframe; set " + tabname + ";run;\n"
                ...
                try:
                    ...
                    saved_file = os.path.join(temp_dir,
                                              'dat_{}.sas7bdat'.format(idx))
                    if not os.path.isfile(saved_file):
                        self.sos_kernel.warn(
                            'Failed to save dataset {} to {}'.format(
                                item, saved_file))
                        continue
